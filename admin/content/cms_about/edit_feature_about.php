...
$id = intval($_GET['id']);
$query = mysqli_query($conn, "SELECT * FROM about_features WHERE id = $id");
$data = mysqli_fetch_assoc($query);

if (!$data) {
    echo "<script>
        Swal.fire({icon: 'error', title: 'Error', text: 'Data tidak ditemukan'});
        window.location='?page=features_about';
    </script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $query = "UPDATE about_features SET icon='$icon', text='$text', sort_order=$sort_order WHERE id=$id";
        if (mysqli_query($conn, $query)) {
            $_SESSION['alert'] = ['type' => 'success', 'title' => 'Berhasil!', 'message' => 'Fitur berhasil diperbarui'];
            header("Location: ?page=features_about");
            exit;
        } else {
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card m-b-30">
                <div class="card-body">
                    <h4 class="mt-0 header-title">Detail Fitur</h4>
                    <p class="text-muted m-b-30 font-14">
                        Perbarui informasi fitur keunggulan yang ditampilkan.
                    </p>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" name="text" id="text" required
                                    value="<?php echo htmlspecialchars($data['text']); ?>" maxlength="255">
                                <small class="form-text text-muted">
                                    <i class="mdi mdi-information-outline"></i> Teks singkat yang menjelaskan keunggulan
                                    (Maks. 255 karakter).
                                </small>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-9">
                                <select class="form-control select2-icon" name="icon" id="icon" style="width: 100%;">
                                    ...
                                    foreach ($icons as $value => $label) {
                                        $selected = ($data['icon'] == $value) ? 'selected' : '';
                                        echo "<option value='$value' $selected>$label</option>";
                                    }
                                    ?>
                                </select>
                                <small class="form-text text-muted mt-2">
                                    <i class="mdi mdi-information-outline"></i> Pilih ikon yang relevan dari daftar.
                                </small>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="number" class="form-control" name="sort_order"
                                    value="<?php echo intval($data['sort_order']); ?>" min="0">
                                <small class="form-text text-muted">
                                    <i class="mdi mdi-information-outline"></i> Semakin kecil angkanya, semakin awal
                                    munculnya.
                                </small>
                            </div>
                        </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="card m-b-30 border-secondary">
                ...
                <div class="card-body">
                    <table class="table table-sm table-borderless mb-0">
                        <tr>
                            <td class="text-muted" width="40%">ID Fitur:</td>
            ...
            <div class="card m-b-30 border-info">
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center p-3 bg-white rounded shadow-sm">
                        <div class="mr-3">
                            <i id="preview-icon"
                                class="<?php echo htmlspecialchars($data['icon']); ?> text-success font-24"></i>
                        </div>
                        <div>
                            <span id="preview-text" class="font-weight-bold text-dark font-16">
                                <?php echo htmlspecialchars($data['text']); ?>
                            </span>
                        </div>
                    </div>
                    <p class="text-center mt-3 text-muted small">
                        Perubahan akan langsung terlihat di sini.
                    </p>
                </div>
            </div>
            ...
            <div class="card m-b-30 border-success">
                ...
                <div class="card-body">
                    <ul class="mb-0 pl-3">
                        <li class="mb-2">Gunakan kalimat singkat & padat (2-4 kata).</li>
...
<script>
    $(document).ready(function () {
        ...
        $('#text').on('input', function () {
            var val = $(this).val();
            $('#preview-text').text(val || 'Teks Fitur');
        });
